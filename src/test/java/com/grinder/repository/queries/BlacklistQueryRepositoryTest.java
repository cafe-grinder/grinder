...
class BlacklistQueryRepositoryTest {
    ...
    @Test
    @DisplayName("블랙리스트 목록 불러오기")
    void findAllBlacklistByMemberEmail() {
        List<BlacklistDTO.findAllResponse> find = blacklistQueryRepository.findAllBlacklistByMemberEmail("test@test.com");

        List<String> expectedMembers = Arrays.asList("test1", "test2");

        Assertions.assertEquals(find.size(), 2);
        //예상 리스트를 돌며 확인
        assertThat(find).extracting("blockedNickname").containsAll(expectedMembers);
    }
}